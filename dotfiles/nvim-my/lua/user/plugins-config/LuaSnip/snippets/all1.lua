...
ls.add_snippets("all", {
	...
	s("class", {
		...
		c(3, {
			t("{"),
            -- sn: Nested Snippet. Instead of a trigger, it has a position,
            -- just like insertNodes. !!! These don't expect a 0-node!!!!
            -- Inside Choices, Nodes don't need a position as the choice node
            -- is the one being jumped to.
			sn(nil, {
				t("extends "),
	...
	s("cond0", {
		...
		condition = function(line_to_cursor, matched_trigger, captures)
			-- optional whitespace followed by //
			-- My.lua.Print(string.format("line: %s", line_to_cursor))
			-- My.lua.Print(string.format("trigger: %s", matched_trigger))
			-- My.lua.Print(string.format("captures: %s", My.lua.Dump(captures)))
			return line_to_cursor == "abc"
		end,
		show_condition = function(line_to_cursor)
			-- optional whitespace followed by //
			-- My.lua.Print(string.format("cond0 line: %s", line_to_cursor))
			return true
		end,
	}),
	...
	s("cond2", {
		...
		show_condition = function(line_to_cursor)
			-- optional whitespace followed by //
			-- My.lua.Print(string.format("cond2 line: %s", line_to_cursor))
			local match = line_to_cursor:match("cond2")
			return true
		end,
	}),
	s("cond3", {
	...
	s("mat3", {
		i(1, { "sample_text" }),
		t(": "),
		m(1, l._1:gsub("[123]", ""):match("%d"), "contains a number that isn't 1, 2 or 3!", "contains only 1, 2 or 3!"),
	}),
