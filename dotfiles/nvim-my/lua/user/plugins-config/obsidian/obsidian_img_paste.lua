...
---@return string
function M.get_clip_check_command()
	local check_cmd
...
function M.clipboard_is_img()
	local content = {}
	for output in assert(io.popen(M.get_clip_check_command())):lines() do
...
function M.save_clipboard_image(path)
	local this_os = util.get_os()
...
M.paste_img = function(opts)
	...
	if not M.clipboard_is_img() then
	...
	-- Paste image.
	local result = M.save_clipboard_image(tostring(path))
